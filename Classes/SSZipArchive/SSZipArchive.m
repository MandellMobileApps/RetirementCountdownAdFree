                ...
                if (readBytes >= 0) {
                    ...
                    else
                    {
                        ...
                        switch (errnoSave) {
                                ...
                                // Just log the error
                            {
                                NSError *errorObject = [NSError errorWithDomain:NSPOSIXErrorDomain
                                                                           code:errnoSave
                                                                       userInfo:nil];
                                DLog(@"[SSZipArchive] Failed to open file on unzipping.(%@)", errorObject);
                            }
                                break;
                        }
                        ...
                        if (isSeriousError) {
                            ...
                            unzCloseCurrentFile(zip);
                            // Log the error
                            DLog(@"[SSZipArchive] Failed to open file on unzipping.(%@)", unzippingError);
...
- (BOOL)writeData:(NSData *)data filename:(nullable NSString *)filename compressionLevel:(int)compressionLevel password:(nullable NSString *)password AES:(BOOL)aes
{
    ...
    zip_fileinfo zipInfo = {};
    [SSZipArchive zipInfo:&zipInfo setDate:[GlobalMethods todayNsDate]];
